...
import (
	"fmt"
	"net"
	"shoeshiner/pkg/efi/efidevicepath"
	"shoeshiner/pkg/efi/efivarfs"
	"slices"
)
...
func Load(efivars string) (*Config, error) {
	...
	for _, fp := range entry.FilePathList {
		...
		case efidevicepath.HardDriveType:
			...
			if hd.SignatureType == efidevicepath.HardDriveMBRSignatureType {
			...
			if hd.SignatureType == efidevicepath.HardDriveGUIDSignatureType {
